...
try:
    ...
    PAPER = [1, 2, 3]
    # RANK = [1,2,3]
    REQ = {(1, 1): 45, (2, 1): 0, (3, 1): 10, (1, 2): 20, (2, 2): 10, (3, 2): 34, (1, 3): 12, (2, 3): 17,
           (3, 3): 28}  # (MACH, PAPER) : TIME
    SEQ = {1: [1, 3, 2], 2: [2, 1, 3], 3: [3, 1, 2]}  # PAPER : [SEQ] paper1

    # create variables
    # RNK = {}
    # for i in RANK:
    #     RNK[i] = m.addVars(product(MACH, PAPER), vtype=GRB.BINARY, name='RANK')
    ...
    for mc in MACH:
        for f, s in combinations(PAPER, 2):
            m.addConstr(START[mc, f] + REQ[mc, f] <= START[mc, s] + SUM_REQ * (1 - MSEQ[mc][f, s]),
                        'constr2' + str(mc) + str(f) + str(s))
            m.addConstr(START[mc, s] + REQ[mc, s] <= START[mc, f] + SUM_REQ * MSEQ[mc][f, s],
                        'constr3' + str(mc) + str(f) + str(s))
    for p in PAPER:
